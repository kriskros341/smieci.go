...
export const PhotoGallery = (props: PhotoGalleryProps) => {
  ...
  const onFocusChange = (index: number) => {
    translateX.value = withTiming(index * -mainPhotoLayoutWidth.value, { duration: 20 });
  ...
  const gesture = Gesture.Pan()
    ...
    .onEnd((event: any) => {
      ...
      tempTranslateX.value = withTiming(indexDelta * -mainPhotoLayoutWidth.value, {}, () => {
      ...
      <Pressable onPress={props.onPhoto}>
        {({ pressed }) => (
          <Animated.View className="aspect-square w-full flex justify-center items-center">
            <View className={cn("p-1 rounded-lg shadow-md shadow-black bg-blue-500", pressed && "opacity-50" )}>
              <AntDesign className="bg-none" name="plus" size={64} color="black" />
            </View>
        ...
        <Pressable onPress={props.onPhoto}>
          {({ pressed }) => (
            <Animated.View className="aspect-square h-full flex justify-center items-center">
              <View className={cn("p-1 rounded-lg shadow-md shadow-black bg-blue-500", pressed && "opacity-50" )}>
                <AntDesign className="bg-none" name="plus" size={40} color="black" />
              </View>
            </Animated.View>
          )}
        </Pressable>
