...
const VerificationPhotoEditor = ({ newPhotoUri, originalPhotoUri, commit }: { newPhotoUri?: string, originalPhotoUri: string, commit: (uri: string) => Promise<void> }) => {
  ...
  const pan = Gesture.Pan()
    .onChange((value) => {
      ...
      if (newValue < totalScreenWidth.value && newValue > totalScreenWidth.value * 0.2) {
  ...
  if (!tempPhotoUri) {
    fabButtons.push(
      <Button title="Zrób zdjęcie" onPress={onPhoto} />
    )
  } else {
    fabButtons.push(
