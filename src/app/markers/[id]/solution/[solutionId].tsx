...
import { Modal, Pressable, StyleSheet, Text, TouchableWithoutFeedback, View, ActivityIndicator } from "react-native";
...
const useContextMenu = ({ items }: useContextMenuOptions) => {
  ...
  const onLayout = (event: any) => {
    event.target?.measure?.((x: any, y: any, width: any, height: any, pageX: any, pageY: any) => {
      setButtonPosition({ x, y: y + height });
    });
  };

  const Trigger = (
    <TouchableOpacity className="flex flex-row" onPress={() => setMenuVisible(true)}>
      <Entypo name="dots-three-vertical" size={24} color="black" onLayout={onLayout} />
    </TouchableOpacity>
  )
  ...
  const Menu = (
      ...
      >
        <TouchableWithoutFeedback className="flex-1" onPress={() => setMenuVisible(false)}>
          <View className="w-full h-full">
            <View style={[styles.menu, { right: buttonPosition.x, top: buttonPosition.y }]}>
              {items.map(({ text, callback }) => (
                <Pressable key={text} onPress={() => { callback(); setMenuVisible(false); }} style={styles.menuItem}>
...
const useSolutionStatusMutation = (solutionId: unknown) => {
  ...
  return useMutation({
    mutationFn: (payload: string) => axios.patch(`/solutions/${solutionId}/status`, { status: payload }),
    onError: (e: any) => {
      Toast.show({
        type: "error",
        text1: "wystąpił błąd",
...
const PreivewMarkerSolution = () => {
  ...
  if (markerData?.pendingVerificationsCount === -1) { // Rozwiązany
    contextMenuItems.push(
      { text: 'Otwórz ponownie', callback: () => option('pending') },
    )
  } else {
    contextMenuItems.push(
      ...
      navigation.setOptions({
        headerRight: () => (
          <View className="flex flex-row gap-4">
            <StatusBadge pendingVerificationsCount={markerData?.pendingVerificationsCount || 0} />
